...
def get_page_movie(page):
    for i in range(0,page):
        start = 20 * i
        print(start)
        url = "https://movie.douban.com/j/chart/top_list?type=13&interval_id=100%3A90&action=&start=" + str(start) + "&limit=20"
        request = urllib.request.Request(url=url,headers=headers)
        response = urllib.request.urlopen(request)
        print('运行到这')
        content = response.read().decode("utf-8")
        with open("豆瓣电影第" + str(i) + "页.json" ,"w",encoding="utf-8") as fp:
            fp.write(content)
